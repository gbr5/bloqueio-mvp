...
export class EasyBot {
  ...
  async selectMove(gameState: GameSnapshot, playerId: number): Promise<Move> {
    ...
    if (this.rng.next() < 0.6) {  // ⚠️ Use seeded RNG
  ...
  async selectBarrier(gameState: GameSnapshot, playerId: number): Promise<Barrier | null> {
    const player = gameState.players.find(p => p.id === playerId);
...
export async function executeBotTurn(
  ...
  try {
    ...
    if (shouldPlaceBarrier) {
      ...
      if (barrier) {
        await placeBarrier(roomCode, barrier.row, barrier.col, barrier.orientation);
        return { success: true };
      }
    }
  ...
  try {
    ...
    if (!room) {
      await db.botMoveJob.update({
        where: { id: jobId },
        data: { status: 'FAILED', error: 'Room not found', completedAt: new Date() }
      });
      return;
    }
    ...
    if (room.currentPlayer !== job.playerId) {
      await db.botMoveJob.update({
        where: { id: jobId },
        data: { status: 'STALE', error: 'Not bot turn', completedAt: new Date() }
      });
      return;
    }
    ...
    if (computeTime > 5000) {
      console.warn(`⚠️ Bot move exceeded 5s: ${computeTime}ms (room: ${job.roomCode})`);
    }
...
export async function afterMoveCommit(roomCode: string): Promise<void> {
  ...
  const currentPlayer = room.players.find(p => p.playerId === room.currentPlayer);
...
const isBotThinking = room?.currentPlayer !== undefined && 
  room.players.find(p => p.playerId === room.currentPlayer)?.playerType !== 'HUMAN' &&
...
export async function createRoomWithBots(config: {
  ...
  const humanCount = config.playerSlots.filter(s => s.type === 'HUMAN').length;
  ...
  private minimax(
    ...
    if (maximizing) {
      ...
      for (const move of orderedMoves) {
        ...
        const eval_ = this.minimax(newState, depth - 1, alpha, beta, false, timeLeft - 100);
      ...
      for (const move of orderedMoves) {
        const newState = this.simulateMove(gameState, currentPlayer.id, move);
        const eval_ = this.minimax(newState, depth - 1, alpha, beta, true, timeLeft - 100);
...
export async function checkInactivePlayers(
  ...
  const inactivePlayers = room.players
    ...
    .map(p => ({
      playerId: p.playerId,
      secondsInactive: Math.floor((now.getTime() - p.lastActivity.getTime()) / 1000)
    }))
...
export function BotReplacementModal({
  ...
  const handleReplace = async () => {
    const result = await replacePlayerWithBot(roomCode, inactivePlayer.playerId, difficulty);
    
    if ('success' in result) {
      toast.success(`Player replaced with ${difficulty.replace('BOT_', '')} bot`);
      onClose();
    }
  };
  ...
  return (
    <Modal open onClose={onClose}>
      <ModalHeader>
        ...
        <ModalDescription>
          "{inactivePlayer.name}" has been inactive for {inactivePlayer.secondsInactive}s.
        </ModalDescription>
      </ModalHeader>
